...
TEST_PRODUCTS = [
    {
        'stripe_product_id': 'there_isnt_a_real_product_id_for_free_tier',
        'stripe_price_id':   'there_isnt_a_real_price_id_for_free_tier',
        'name':              'Free',
        'tier':              'free',
        'amount':            0,
    ...
    {
        'stripe_product_id': 'prod_Ur7SEvl41ewdeX',
        'stripe_price_id':   'price_1TrPDNGUL6ygYkqqQUP5PIIk',
    ...
    {
        'stripe_product_id': 'prod_Ur7TvFJzszwFWq',
        'stripe_price_id':   'price_1TrPEBGUL6ygYkqqHIVNCJri',
    ...
    {
        'stripe_product_id': 'prod_Ur7UOijTsiRpY1',
        'stripe_price_id':   'price_1TrPFFGUL6ygYkqqMTNqNrhk',
...
def upgrade():
    ...
    op.create_check_constraint(
        'ck_users_subscription_tier', 'users',
        "subscription_tier IN ('free', 'premium', 'professional', 'enterprise')",
    )
 
    # -------------------------------------------------------------------------
    # updated_at trigger function (shared by all tables below)
    # -------------------------------------------------------------------------
    op.execute("""
    ...
    op.create_table(
        ...
        sa.Column('billing_interval', sa.String(20), nullable=False),                  # renamed from 'interval' (reserved word)
    ...
    op.create_table(
        ...
        # FK to products.id (UUID) — stable, Stripe-ID-independent relational link
        ...
        # Stripe IDs kept as plain strings for webhook reconciliation — not FKs
    ...
    # -------------------------------------------------------------------------
    # Tier sync trigger
    # Fires after any INSERT or UPDATE on subscriptions and keeps
    # users.subscription_tier consistent with the active subscription.
    #
    # active / trialing               -> promote to the product's tier
    # canceled / incomplete_expired
    #   / unpaid / paused             -> demote to 'free'
    # past_due                        -> no-op (grace period; webhook handles final outcome)
    # -------------------------------------------------------------------------
    op.execute("""
